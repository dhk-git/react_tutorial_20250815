  ...
  return (
    <div className="container mx-auto px-4 py-32 max-w-7xl">
      <div className="relative rounded-2xl overflow-hidden shadow-2xl mb-24">
        <img src={CompanyImage} className="w-full h-full object-cover" alt="" />
      ...
      <div className="mb-24 max-w-4xl mx-auto">
        <h2 className="text-4xl font-bold mb-8 text-lsate-800 text-center">
          회사 소개
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-24">
        {abouts.map((about, idx) => (
          ...
          >
            ...
            <p className="text-gray-600 text-lg">{about.desc}</p>
          </div>
        ))}
      </div>
      <div className="mb-24 max-w-4xl mx-auto text-center">
        ...
        <p className="text-2xl leading-relaxed text-gray-600 font-light">
          "2030년까지 글러벌 시장을 선호다는 기슬혁신 기업으로 도력하는, <br />{" "}
          더 나는 세상을 만드는데 기여하겠습니다."
        </p>
      </div>
            ...
            <div key={idx} className={`flex items-center gap-8 ${idx % 2 === 0 ? "flex-row" : "flex-row-reverse"}`}>
              <div className="w-1/2 text-center">
                <div className="bg-white rounded-xl shadow-lg p-8 hover:shadow-2xl transition-all duration-300 border border-gray-100">
                  <h3 className="text-2xl font-bold mb-3 text-indigo-600">{history.year}</h3>
                  <p className="text-gray-700 text-lg">{history.event}</p>
                </div>
              </div>
